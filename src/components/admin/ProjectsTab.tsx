...
const ProjectsTab = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      live_url: form.live_url || null, github_url: form.github_url || null, featured: form.featured,
    };
    ...
    if (mediaFiles.length > 0 && projectId) {
      ...
      for (let i = 0; i < mediaFiles.length; i++) {
        ...
        await supabase.from("project_media").insert([{ project_id: projectId, type: file.type.startsWith("video") ? "video" : "image", url: urlData.publicUrl, sort_order: i }]);
      }
      setUploading(false);
    }
  ...
  const startEdit = async (project: Project) => {
    ...
    setForm({ title: project.title, description: project.description, tags: project.tags.join(", "), live_url: project.live_url || "", github_url: project.github_url || "", featured: project.featured });
  ...
  const deleteMedia = async (mediaId: string) => {
    ...
    else { toast.success("Media deleted"); setExistingMedia(prev => prev.filter(m => m.id !== mediaId)); }
  };

  const resetForm = () => {
    setShowForm(false); setEditing(null);
    setForm({ title: "", description: "", tags: "", live_url: "", github_url: "", featured: false });
    setMediaFiles([]); setExistingMedia([]);
  };
    ...
    <div className="space-y-6">
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="glass-card p-6 space-y-4">
          ...
          <Textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="Description" className="bg-zinc-900 border-white/10 resize-none" required />
          <Input value={form.tags} onChange={e => setForm({ ...form, tags: e.target.value })} placeholder="Tags (comma-separated)" className="bg-zinc-900 border-white/10" />
          <Input value={form.live_url} onChange={e => setForm({ ...form, live_url: e.target.value })} placeholder="Live URL" className="bg-zinc-900 border-white/10" />
          <Input value={form.github_url} onChange={e => setForm({ ...form, github_url: e.target.value })} placeholder="GitHub URL" className="bg-zinc-900 border-white/10" />
