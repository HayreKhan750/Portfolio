...
-- Fixed version without has_role() dependency
...
-- Public read, authenticated users can modify (you can restrict further later)
CREATE POLICY "Anyone can view contact_methods" ON public.contact_methods 
  FOR SELECT USING (true);

CREATE POLICY "Authenticated users can insert contact_methods" ON public.contact_methods 
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can update contact_methods" ON public.contact_methods 
  FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can delete contact_methods" ON public.contact_methods 
...
-- Public read, authenticated users can modify
CREATE POLICY "Anyone can view project_media" ON public.project_media 
  FOR SELECT USING (true);

CREATE POLICY "Authenticated users can insert project_media" ON public.project_media 
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can update project_media" ON public.project_media 
  FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated users can delete project_media" ON public.project_media 
  FOR DELETE USING (auth.role() = 'authenticated');
